...
import com.wakacommerce.common.web.WakaRequestFilter;
...
public class WakaAdminRequestProcessor extends AbstractWakaWebRequestProcessor {
    ...
    public void process(WebRequest request) throws SiteNotFoundException {
        ...
        // Note: The currencyResolver will set the currency on the WakaRequestContext but 
    ...
    protected void prepareProfile(WebRequest request, WakaRequestContext brc) {
    ...
    protected void prepareCatalog(WebRequest request, WakaRequestContext brc) {
        AdminUser adminUser = adminRemoteSecurityService.getPersistentAdminUser();
        if (adminUser == null) {
            //clear any catalog
            if (BLCRequestUtils.isOKtoUseSession(request)) {
    ...
    protected void prepareSandBox(WebRequest request, WakaRequestContext brc) {
        AdminUser adminUser = adminRemoteSecurityService.getPersistentAdminUser();
        if (adminUser == null) {
            //clear any sandbox
            if (BLCRequestUtils.isOKtoUseSession(request)) {
